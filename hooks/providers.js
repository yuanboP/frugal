...
const PROVIDERS = [
  ...
  { name: 'Firebase', pattern: cmd('firebase'),
    hint: 'Blaze has NO hard cap and budgets only EMAIL with 24-48h billing lag. Firestore bills per document READ — a useEffect/crawler loop hit $5k/mo at 100 users; check onWrite triggers for recursion, set functions maxInstances, and restrict browser keys (Hosting serves your apiKey publicly at /__/firebase/init.json)' },
  ...
  { name: 'Twilio', pattern: cmd('twilio'),
    hint: 'SMS pumping turns open OTP endpoints into $2k-$18k/day fraud bills (refunds partial at best). Before shipping: per-IP/per-number rate limits on sends, lock Geo Permissions, enable Fraud Guard, and cap or DISABLE auto-recharge (it refilled one victim\'s card 7x)' },
  { name: 'GPU cloud', pattern: cmd('runpodctl|vastai|modal'),
    hint: 'GPU time bills wall-clock even when idle or crashed ($23-$340 per forgotten night; warm endpoints ~$1,800/mo). Set auto-stop/TTL before long runs, checkpoint often, and verify zero running pods before ending the session' },
  { name: 'Observability/logs', pattern: cmd('datadog-agent|aws\\s+logs\\s+create-log-group'),
    hint: 'log/trace ingest bills per GB (CloudWatch ~$0.50/GB; Datadog bills ingest AND indexing separately) — $18k-$50k/mo blowups are the largest recurring cluster on X. Never ship DEBUG or full request bodies from prod, set retention on every log group, sample spans, keep IDs out of metric tags' },
